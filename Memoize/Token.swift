...
class Token
{
   ...
   */
   public func getToken(account: String) -> String?
   {
      let token = Passcode.loadPasscode(account)
      guard !token.isEmpty
              else {return nil}
   ...
   private func fetchToken(_ serverAddress: String)
   {
      ...
      OAUTH.authorize(deviceToken: "Memoize", grantType: "client_credentials",
                      success: {credential, response, parameters in
                         ...
                         self.saveToken(token: credential.oauthToken, account: "MemoizeToken")
                      },
   ...
   */
   func saveToken(token: String, account: String)
   {
      ...
      Passcode.saveToken(token, account)
   }
   
   func removeToken(account: String)
   {
      Passcode.removeToken(account)
   }
}
